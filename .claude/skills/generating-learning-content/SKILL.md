...
interviewQuestions:
  - level: "junior"               # junior | mid | senior | staff  — NO id: field, auto-generated by build
    ...
    relatedQuestions: ["b1t1q2"]   # predictable format: b{block}t{topic}q{1-based index}
  - level: "mid"
...
- `relatedQuestions`: IDs of natural follow-up questions (can cross topics). IDs are **auto-generated** by the build script as `b{blockId}t{topic}q{N}` (1-based N). Never write an `id:` field on a question — use the formula to predict IDs when cross-referencing. Example: in block 3, topic 2, question 1 is `b3t2q1`; from another file, reference it as `"b3t2q1"`
...
- [ ] `relatedQuestions` use correct auto-generated format `b{block}t{topic}q{N}` — NO `id:` field on questions
...
| Writing `id:` on a question | IDs are auto-generated — never add `id:` to a question; use `b{block}t{topic}q{N}` only in `relatedQuestions` |
